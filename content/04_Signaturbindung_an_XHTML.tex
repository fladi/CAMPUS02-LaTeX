...
\xmlelem{KeyInfo} sowie \xmlelem{SignatureValue}. Um diese Attribute von \gls{xhtml} zu trennen müsste ein eigener \gls{xml-ns} geschaffen werden. \todo
...
\xmlelem{Transformation}-Elementen ausgegangen wird. Die Selektion von einzelenen Elementen je Signatur ist so nicht mehr möglich.
...
\gls{xml-dsig}-Dokument mit einen \gls{url} in einem Attribut aus einem eigenen \gls{xml-ns} referenziert. Dieses \gls{xml-dsig} bietet dann wieder die volle
...
\gls{xhtml}-Elements mit dem entsprechenden Attribut befinden. \todo
