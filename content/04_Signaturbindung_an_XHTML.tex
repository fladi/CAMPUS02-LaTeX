...
\xmlelem{Reference}-Elementen auflösbar sind. Mit einem Mikroformat, das mit fest vorgegebenen Transformationen arbeitet, könnten die verbleibenden variablen
...
\xmlelem{KeyInfo} sowie \xmlelem{SignatureValue} selbst. 
Für \xmlelem{Reference} kann angenommen werden, dass es auf die Informationen innerhalb des \gls{xhtml}-Elements verweist, in dem die Attribute des Mikroformats
angehängt sind. Eine \xmlelem{Transformation} für \gls{c14n} und \gls{sha1} als \xmlelem{DigestMethod} können ebenfalls vorgegeben werden. 

\lstinputlisting[language=HTML,caption={Verwendung von Mikroformaten},label=lst:mikroformat,emph={signature,key,xml-dsig}]{source/mikroformate.html}
