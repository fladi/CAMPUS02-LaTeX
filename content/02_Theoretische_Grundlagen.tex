...
\section{Bereiche in \glsentrytext{xhtml}-Dokumenten}
...
\gls{openpgp} und \gls{x509} verfolgen hierbei jeweils unterschiedliche Ansätze, um Vertrauensbeziehungen zwischen Schlüsseln zu definieren
\cite{krypto-pki-internet}.

\subsection{\glsentrydesc{wot}}
\index{Web Of Trust}
...
\subsection{\glsentrydesc{ht}}
\index{Hierachical Trust}
...
Wurzel, der Root-\gls{ca} des Baumes, zu einem der Zertifikate an einem Ast führt wird als \gls{cot} bezeichnet. \fref{fig:ht} stellt einen solchen Baum mit
mehreren \glspl{cot} dar.

Die Anzahl der Ebenen in einem \gls{ht}-Baum unterliegt keiner Beschränkung, jedoch muss einem Zertifikat explizit das Recht eingeräumt werden, andere
...
\caption{Schematische Darstellung eines \texorpdfstring{\protect\Glsentryname{ht}}{HT}}
\label{fig:ht}
\end{figure}

\section{\glsentrytext{xml-dsig}}
\index{XML-DSig}
...
machen \cite{krypto-pki-internet}. In \fref{chap:XML-DSig} wird näher auf die Arbeitsweise von \gls{xml-dsig} eingegangen.
