...
\gls{x509}-Zertifikate sein, die bereits in der zu signierenden \gls{xml}-Datei enthalten sind. Das Verhalten von \xmlelem{RetrievalMethod} entspricht bis auf
das Fehlen der Elemente \xmlelem{DigestMethod} und \xmlelem{DigestValue} dem des Elements \xmlelem{Reference} welches in \fref{sec:Signaturbindung:Referenzen}
...
Lage, zwei weitere Elemente aufzunehmen: \xmlelem{PGPKeyID} nimmt die Schlüssel-ID eines \gls{openpgp}-Schlüsselpaares auf, welche aus 8 \glspl{oktett} besteht und die
...
\lstinputlisting[float=bt,language=XML,caption={Beispiel einer Struktur für \protect\xmlelem{X509Data}},label=lst:xml-dsig-keyinfo-x509,emph={X509Data,X509IssuerSerial,X509SKI,X509Certificate,X509SubjectName}]{source/xml-dsig-keyinfo-x509.xml}
...
\xmlelem{SignedInfo} signiert. Die Verbindung zwischen den zu schützdenen Informationen und der Signatur wird in \fref{sec:XML-DSig:Referenzen} näher erläutert.

Da \xmlelem{SignedInfo} selbst diesen Problemen mit unterschiedlichen Formatierungen unterliegt, wird über das obligatorische Attribut
\xmlattr{CanonicalizationMethod}{Algorithm} des Elements \xmlelem{CanonicalizationMethod} ein Algorithmus angegeben, der
die Umwandlung von \xmlelem{SignedInfo} in ein kanonisches Format festlegt. Innerhalb von \xmlelem{SignedInfo} muss das Element \xmlelem{CanonicalizationMethod}
...
Element \xmlelem{Reference} zur Verfügung gestellt, welches mehrfach in \xmlelem{SignedInfo} enthalten sein kann. Jede Vorkommen dieses Elements stellt einen Verweis auf eine
signierte Information dar. Der Verweis ist in Form eines \gls{uri} im optionalen Attribut \xmlattr{Reference}{URI} hinterlegt. Wenn \xmlattr{Reference}{URI}
...
über \gls{http} referenziert werden ist in \fref{lst:xml-dsig-signedinfo-reference} in der \fref{lin:xml-dsig-signedinfo-reference-http} zu sehen.
