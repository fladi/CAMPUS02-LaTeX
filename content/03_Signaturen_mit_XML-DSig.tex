...
das Fehlen der Elemente \xmlelem{DigestMethod} und \xmlelem{DigestValue} in seinem Verhalten dem Element \xmlelem{Reference}, welches in 
...
Form überführt, welche dann schließlich die zu signierenden Information bildet. Dadurch wird sichergestellt, dass die Signatur unabhängig von der Formatierung 
überprüft werden kann und nur Änderungen an der Struktur oder dem Inhalt die Integrität der Signatur beeinflussen. 
...
\lstinputlisting[language=XML,
                 ...
                 emph={Reference}]{source/xml-dsig-signedinfo-reference.xml}
...
auch reiner Text oder binäre Daten sein. \fref{fig:xslt-processing} zeigt den schematischen Aufbau einer solchen Transformationskette.
...
Für die Verwendung in \gls{xml-dsig} ist vorgesehen, dass das anzuwendende Stylesheet unter dem Element \xmlelem{Transform} in einem eigenen \gls{xml-ns} abgelegt wird.
Je Transformation darf nur ein Stylesheet definiert werden. Durch die Verwendung von \xmlelem{xsl:include} oder \xmlelem{xsl:import} im Stylesheet ist es möglich 
...
direkt anzubinden, wie dies bei \xmlelem{Reference} der Fall ist. 
...
\lstinputlisting[float=p,
                 language=XML,
                 caption={\texorpdfstring{\protect\Glsentryname{xslt}}{XSLT}-Transformation in einer Signatur},
